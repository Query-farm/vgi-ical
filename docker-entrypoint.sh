...
#
# Dispatch the single vgi-ical image into one of its transports:
...
#                    $VGI_UNIX_SOCK or /tmp/vgi-ical.sock).
...
#
# The worker is stateless (iCal4j parser over bytes/paths), so there is no /data
# to create and no state env to wire — each mode just exec's the fat JAR.
set -e
...
case "${1:-http}" in
  ...
  unix)
    shift 2>/dev/null || true
    SOCK="${1:-${VGI_UNIX_SOCK:-/tmp/vgi-ical.sock}}"
